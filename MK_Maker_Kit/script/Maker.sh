...
while true; do
	...
	read -p "What programmer should we use? Tested versions include usbtiny, usbasp, avrisp2 [usbasp]: " programmer
	[ -z $programmer ] && programmer=usbasp
	...
	avrcommand="avrdude -P usb -c $programmer -p attiny25 -F -e -U flash:w:pr.hex -U eeprom:w:$tempfile"
	while true; do
